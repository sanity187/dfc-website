...
interface LearnRequirementsSectionProps {
  locale: Locale;
}

export function LearnRequirementsSection({ locale }: LearnRequirementsSectionProps) {
  const { requirements } = learnContent;
  ...
  return (
    <section aria-label="Student Prerequisites and Eligibility" className="flex flex-col gap-10">
      <SectionHeading
        eyebrow={t(requirements.eyebrow, locale)}
        title={t(requirements.title, locale)}
        subtitle={t(requirements.subtitle, locale)}
        align="center"
      />
