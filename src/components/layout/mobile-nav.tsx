...
import { Menu, X, ChevronDown, Phone, Calendar, ArrowRight, ShieldCheck, Sparkles, Plane } from "lucide-react";
...
export function MobileNav({ locale }: MobileNavProps) {
  ...
  const drawerContent = isOpen && mounted ? (
    <div
      className="fixed inset-0 z-50 flex flex-col bg-background text-foreground animate-in fade-in-20 duration-200"
      role="dialog"
      aria-modal="true"
      aria-label="Mobile Navigation"
    >
      {/* Top Header Bar: Clean Logo + Close Button ONLY */}
      <div className="flex h-20 shrink-0 items-center justify-between border-b border-line px-4 sm:px-6 bg-background/95 backdrop-blur-xl">
        <BrandLockup locale={locale} compact />

        <button
          type="button"
          onClick={closeDrawer}
          className="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-xl border border-line bg-panel text-ink shadow-xs transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 flex flex-col gap-5">
        ...
        <div className="flex items-center justify-between rounded-xl border border-line bg-panel/70 p-2.5">
          <div className="flex items-center gap-1.5 text-[11px] font-bold text-dim">
        ...
        <div className="flex flex-col divide-y divide-line/70 rounded-2xl border border-line bg-panel shadow-xs overflow-hidden">
            ...
            return (
              <div key={category.id} className="flex flex-col">
                <button
                  ...
                  className="flex items-center justify-between p-4 text-left font-bold text-ink cursor-pointer hover:bg-muted/40 transition-colors"
                >
                    ...
                    {category.links.map((link, idx) => {
                      ...
                      return (
                        <Link
                          ...
                          onClick={closeDrawer}
                          className="flex flex-col rounded-xl border border-line/50 bg-background/80 p-3 transition-colors hover:bg-muted/70"
                        >
          ...
          {megaMenuContent.directLinks.map((link) => (
            <Link
              ...
              href={localizedPath(locale, link.href)}
              onClick={closeDrawer}
              className="flex items-center justify-between p-4 text-base font-bold text-ink hover:bg-muted/40 transition-colors"
            >
        ...
        <div className="rounded-2xl border border-secondary/30 bg-linear-to-br from-primary/10 via-panel to-secondary/10 p-5 shadow-sm">
          ...
          <a
            ...
            className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-secondary py-3.5 text-sm font-black text-secondary-foreground shadow-md transition-transform active:scale-[0.98]"
          >
          ...
          <a
            ...
            className="mt-2.5 flex w-full items-center justify-center gap-2 rounded-xl border border-line bg-panel py-3 text-xs font-bold text-ink transition-colors hover:bg-muted"
          >
  ...
  return (
    <div className="lg:hidden">
      <button
        ...
        className="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-lg border border-line bg-panel text-ink shadow-xs transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
